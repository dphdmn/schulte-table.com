...
function bornbox(text,posx,posz) {
	    ...
	    var size = dynamicTexture2.getSize();
            ...
            textureContext2.save();
            // Use color from tileColors map, default to white if not found
            textureContext2.fillStyle = tileColors.has(text) ? tileColors.get(text) : "white";
            textureContext2.fillRect(0, 0, size.width, size.height);
...
//            box[BOX_ID] = BABYLON.Mesh.CreateBox("box_"+BOX_ID, 0.9, scene);
	    ...
	    var size = dynamicTexture2.getSize();
            ...
            textureContext2.save();
            // Use color from tileColors map, default to white if not found
            textureContext2.fillStyle = tileColors.has(BOX_ID) ? tileColors.get(BOX_ID) : "white";
            textureContext2.fillRect(0, 0, size.width, size.height);
